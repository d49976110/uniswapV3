    ...
    SelfPermit
{
    ...
    {
        IUniswapV3Pool pool;
        // 添加流動性，會call pool中的mint，這邊主要是處理erc20 的部分
        // LiquidityManagement.addLiquidity -> Pool.mint
        (liquidity, amount0, amount1, pool) = addLiquidity(
            AddLiquidityParams({
                token0: params.token0,
                token1: params.token1,
                fee: params.fee,
                recipient: address(this), //流動性的所屬人地址
                tickLower: params.tickLower, //流動性的價格下限（以 token0 計價），這里傳入的是 tick index
                tickUpper: params.tickUpper, //流動性的價格上限（以 token0 計價），這里傳入的是 tick index
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        position.tokensOwed0 +=
            uint128(amount0) +
            uint128(FullMath.mulDiv(feeGrowthInside0LastX128 - position.feeGrowthInside0LastX128, positionLiquidity, FixedPoint128.Q128));
        position.tokensOwed1 +=
            uint128(amount1) +
            uint128(FullMath.mulDiv(feeGrowthInside1LastX128 - position.feeGrowthInside1LastX128, positionLiquidity, FixedPoint128.Q128));
